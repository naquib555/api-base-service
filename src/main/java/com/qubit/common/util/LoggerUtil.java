...
public class LoggerUtil implements Serializable {
    ...
    private static void initLogger() {
        ...
        File logDirectory = new File(System.getProperty("user.home")+String.format("/logs/?", appName));
        ...
        logProperties.setProperty("log4j.appender.stdout.layout.ConversionPattern", "<CCM> ${LOG_PATTERN}");
    ...
    private static void configureLogger(Properties logProperties, String loggerName) {
        ...
        if (loggerName.equalsIgnoreCase(appName)) {
            logProperties.setProperty("log4j.rootLogger", "${LOG_LEVEL}, stdout, " + loggerName);
            logFileName = "${LOG_PATH}" + File.separator + String.format("?.log", appName);
